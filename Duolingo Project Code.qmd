...
user_test_acc <- duodata |> 
  filter(session == 'test') |> 
  group_by(user, session_id) |> 
  summarize(avg_test_accuracy_by_session = mean(label), 
            .groups = 'keep') |>
  ungroup() |> 
  dplyr::select(-session_id) |>
  group_by(user) |> 
  summarize(avg_test_accuracy = mean(avg_test_accuracy_by_session), 
            .groups = 'keep') |> 
  ungroup()

#duodata <- 
left_join(duodata, user_test_acc, 
          by = join_by(user)) |> 
...
user_lesson_acc <- duodata |> 
  ...
  group_by(user, session_id) |> 
  summarize(avg_lesson_accuracy_by_session = mean(label), 
            .groups = 'keep') |>
  ungroup() |> 
  dplyr::select(-session_id) |>
  group_by(user) |> 
  summarize(avg_lesson_accuracy = mean(avg_lesson_accuracy_by_session), 
            .groups = 'keep') |> 
  ungroup()

#duodata <- 
left_join(duodata, user_lesson_acc, 
          by = join_by(user)) |> 
...
duodata |> 
  ...
  group_by(user, session_id) |> 
  summarize(avg_practice_accuracy_by_session = mean(label), 
            .groups = 'keep') |>
  ungroup() |> 
  dplyr::select(-session_id) |>
  group_by(user) |> 
  summarize(avg_practice_accuracy = mean(avg_practice_accuracy_by_session), 
            .groups = 'keep') |> 
  ungroup()

#duodata <- 
left_join(duodata, user_practice_acc, 
          by = join_by(user)) |> 
...
user_rever_t_acc <- duodata |> 
  ...
  group_by(user, session_id) |> 
  summarize(avg_rever_t_accuracy_by_session = mean(label), 
            .groups = 'keep') |>
  ungroup() |> 
  dplyr::select(-session_id) |>
  group_by(user) |> 
  summarize(avg_reverse_trans_accuracy = mean(avg_rever_t_accuracy_by_session), 
            .groups = 'keep') |> 
  ungroup()

#duodata <- 
left_join(duodata, user_rever_t_acc, 
          by = join_by(user)) 
...
duodata |> 
  ...
  group_by(user, session_id) |> 
  summarize(avg_rever_tap_accuracy_by_session = mean(label), 
            .groups = 'keep') |>
  ungroup() |> 
  dplyr::select(-session_id) |>
  group_by(user) |> 
  summarize(avg_reverse_tap_accuracy = mean(avg_rever_tap_accuracy_by_session), 
            .groups = 'keep') |> 
  ungroup()

#duodata <- 
left_join(duodata, user_rever_tap_acc, 
          by = join_by(user)) |> 
...
duodata |> 
  ...
  group_by(user, session_id) |> 
  summarize(avg_listen_accuracy_by_session = mean(label), 
            .groups = 'keep') |>
  ungroup() |> 
  dplyr::select(-session_id) |>
  group_by(user) |> 
  summarize(avg_listen_accuracy = mean(avg_listen_accuracy_by_session), 
            .groups = 'keep') |> 
  ungroup()

#duodata <- 
left_join(duodata, user_listen_acc, 
          by = join_by(user)) |> 
...
levels(train$label) <- make.names(levels(train$label))

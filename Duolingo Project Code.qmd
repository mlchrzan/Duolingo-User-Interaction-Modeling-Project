...
#Maybe round days? Need a row that is just user and 1 day for lag strategy to work
  #Summarize?? 
duodata |> 
  group_by(user, session_id) |> 
  mutate(day_gap = days - dplyr::lag(days), 
         day_gap = if_else(is.na(day_gap), 0, day_gap)) |>
  dplyr::select(user, session_id, days, day_gap) 
...
#Should we remove variables related to tokens-level info? 
```

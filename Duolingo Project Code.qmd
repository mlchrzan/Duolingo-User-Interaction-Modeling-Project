...
#Saving to unstandardize the results later
...
t(var$coord) |> 
  ...
  geom_col(aes(y = value,
               x = variable, 
               fill = value > 0)) +
  scale_fill_manual(values = c('yellow3', "#7ac70c")) +
...
ggplot(aes(x = engagement_metrics, 
  ...
  labs(x = "Engagement Metric", 
       ...
       subtitle = 'Examining Defining Characteristics of Each Cluster') +
...
ggplot(cluster_averages, aes(x = engagement_metrics, 
  ...
  labs(x = "Engagement Metric", 
       y = "Value", 
       title = "Engagement Metrics by Cluster", 
       subtitle = 'Examining Defining Characteristics by Variable') +
  theme(legend.position = "none")
```
...
results <- bind_rows(results, 
                     ...
                     left_join(m_boost_rf_1$finalModel$tuneValue, 
                               m_boost_rf_1$results, 
                               by = join_by(nrounds, max_depth, eta, gamma, colsample_bytree, min_child_weight, subsample))))
...
results <- bind_rows(results, 
                     ...
                     left_join(m_boost_rf_pca$finalModel$tuneValue, 
                               m_boost_rf_pca$results, 
                               by = join_by(nrounds, max_depth, eta, gamma, colsample_bytree, min_child_weight, subsample))))
...
results <- bind_rows(results, 
                     ...
                     left_join(m_boost_rf_hc$finalModel$tuneValue, 
                               m_boost_rf_hc$results, 
                               by = join_by(nrounds, max_depth, eta, gamma, colsample_bytree, min_child_weight, subsample))))
...
results <- bind_rows(results, 
                     ...
                     left_join(m_boost_rf_both$finalModel$tuneValue, 
                               m_boost_rf_both$results, 
                               by = join_by(nrounds, max_depth, eta, gamma, colsample_bytree, min_child_weight, subsample))))
...
```{r}
#Edits for Visualization
results <- results |> 
  mutate(samp = case_when(samp == 'train_samp1' ~ "Raw Data", 
                          samp == 'train_samp_pca' ~ "PCA Dimensions", 
                          samp == 'train_samp_clust' ~ "HC Clusters", 
                          samp == 'train_samp_both' ~ "Both Reductions"), 
         min_rmse = 1 - RMSE, 
         unstand_RMSE = RMSE*response_sd + response_mean,
         samp = as.factor(samp)) 
```

```{r metrics-table-model}
results |> 
  dplyr::select(samp, unstand_RMSE, RMSE, Rsquared) |> 
  arrange(RMSE)
```

```{r rmse-table-predictions}
predicted_drop_train_1 <- predict(m_boost_rf_1, train)
predicted_drop_test_1 <- predict(m_boost_rf_1, test)

predicted_drop_train_pca <- predict(m_boost_rf_pca, train)
predicted_drop_test_pca <- predict(m_boost_rf_pca, test)

predicted_drop_train_hc <- predict(m_boost_rf_hc, train)
predicted_drop_test_hc <- predict(m_boost_rf_hc, test)

predicted_drop_train_both <- predict(m_boost_rf_both, train)
predicted_drop_test_both <- predict(m_boost_rf_both, test)

rmse <- tibble()

rmse <- bind_rows(rmse,
                  tibble(predictors = 'Raw Data', 
                         rmse_train = rmse(train$drop_day, predicted_drop_train_1), 
                         rmse_test = rmse(test$drop_day, predicted_drop_test_1)))

rmse <- bind_rows(rmse,
                  tibble(predictors = 'PCA', 
                         rmse_train = rmse(train$drop_day, predicted_drop_train_pca), 
                         rmse_test = rmse(test$drop_day, predicted_drop_test_pca)))

rmse <- bind_rows(rmse,
                  tibble(predictors = 'HC Clusters', 
                         rmse_train = rmse(train$drop_day, predicted_drop_train_hc), 
                         rmse_test = rmse(test$drop_day, predicted_drop_test_hc)))

rmse <- bind_rows(rmse,
                  tibble(predictors = 'Both Reductions', 
                         rmse_train = rmse(train$drop_day, predicted_drop_train_both), 
                         rmse_test = rmse(test$drop_day, predicted_drop_test_both)))


rmse <- rmse |> 
  mutate(unstand_train = rmse_train*response_sd + response_mean, 
         unstand_test = rmse_test*response_sd + response_mean) |> 
  arrange(rmse_test)

rmse
```

```{r rsquared-table-predictions}
#Using Predictions from the RMSE chunk

rsquared <- function(response, predictions, na.rm = FALSE) {
...
rsquareds <- bind_rows(rsquareds,
                  tibble(predictors = 'Raw Data', 
                         rsq_train = rsquared(train$drop_day, predicted_drop_train_1), 
                         rsq_test = rsquared(test$drop_day, predicted_drop_test_1)))

rsquareds <- bind_rows(rsquareds,
                  tibble(predictors = 'PCA', 
                         rsq_train = rsquared(train$drop_day, predicted_drop_train_pca), 
                         rsq_test = rsquared(test$drop_day, predicted_drop_test_pca)))

rsquareds <- bind_rows(rsquareds,
                  tibble(predictors = 'HC Clusters', 
                         rsq_train = rsquared(train$drop_day, predicted_drop_train_hc), 
                         rsq_test = rsquared(test$drop_day, predicted_drop_test_hc)))

rsquareds <- bind_rows(rsquareds,
                  tibble(predictors = 'Both Reductions', 
                         rsq_train = rsquared(train$drop_day, predicted_drop_train_both), 
                         rsq_test = rsquared(test$drop_day, predicted_drop_test_both)))


rsquareds |> arrange(desc(rsq_test))
```

```{r final-metrics-table}
left_join(rmse, rsquareds, 
          by = join_by(predictors))
```
...
results |> 
  ggplot(aes(x = forcats::fct_reorder(samp, min_rmse), 
             y = min_rmse)) + 
  geom_col() + 
  geom_col(data = results |> filter(samp %in% c("PCA Dimensions", 
                                                     "Both Reductions")), 
           fill = "#7ac70c") + 
  geom_text(aes(label = round(min_rmse, 4)), 
            position = position_dodge(width = 0.9), 
...
train_predictions <- tibble(predicted_drop = predicted_drop_train_both,
                            drop = train$drop_day, 
                            source = 'train')

test_predictions <- tibble(predicted_drop = predicted_drop_test_both,
                           drop = test$drop_day,
                           source = 'test')

all_predictions <- bind_rows(train_predictions, test_predictions)

#Graph Test v Train, UNstandardized
all_predictions |> 
  ...
  ggplot(aes(x = drop,
           y = predicted_drop, 
           color = source)) +
  scale_color_manual(values = c("#7ac70c", 'yellow3')) +
  geom_point() +
  ...
  theme_minimal() + 
  theme(legend.position = 'bottom') +
  labs(title = 'Analyzing Prediction Accuracy', 
       subtitle = 'Actual v Predicted Drop Day for Training and Testing Data', 
       x = 'Actual Drop Day', 
       y = 'Predicted Drop Day', 
       color = 'Dataset')
```
...
```{r}
# plot(gmm_preds, what = 'uncertainty')

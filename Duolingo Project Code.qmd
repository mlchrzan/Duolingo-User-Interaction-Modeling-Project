...
#Substituting extreme values with seemingly reasonable maximums
  ...
  #Removing folks with logging issues (time < 1) as noted in the documentation
duodata <- duodata |> 
  filter(time > 1)
  #Replacing users who used the app longer than 14 hours with 14 hours as their 
...
#Averge Number of Exercises per Session and Number of Exercises by Section 
duodata <- duodata |> 
  group_by(session_id) |> 
...
#Averge Number of Exercises per Day and Number of Exercises by Section 
duodata <- duodata |> 
  group_by(session_id) |> 
  mutate(num_exercises_this_session = n_distinct(exercise_index)) |> 
  ungroup() |> 
...
tot_exer <- duodata |> 
  group_by(user) |> 
  summarize(tot_exercises = n_distinct(exercise_id)) |> 
  ungroup()
...
tot_sess <-  duodata |> 
  group_by(user) |> 
  summarize(tot_sessions = n_distinct(session_id)) |> 
  ungroup()
...
n_test <- duodata |> 
  filter(session == 'test') |> 
  group_by(user) |>
  summarize(tot_test = n_distinct(session_id)) |> 
  ungroup()
...
n_practice <- duodata |> 
  ...
  group_by(user) |>
  summarize(tot_practice = n_distinct(session_id)) |> 
  ungroup()
...
n_lesson <- duodata |> 
  ...
  group_by(user) |>
  summarize(tot_lesson = n_distinct(session_id)) |> 
  ungroup()
...
#THIS WILL ONLY CATCH IT IF THE SESSION GOES OVER A DAY LONG
duodata |> 
  group_by(user, session_id) |> 
  mutate(day_gap = days - dplyr::lag(days), 
         day_gap = if_else(is.na(day_gap), 0, day_gap)) |>
  dplyr::select(session_id, days, day_gap)

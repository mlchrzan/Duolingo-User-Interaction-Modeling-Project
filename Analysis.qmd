...
engagement_metrics_scaled <- engagement_metrics |> mutate(across(where(is.numeric), scale))
```

```{r PCA}
pca <- PCA(engagement_metrics_scaled, ncp = 6, scale.unit = TRUE)
var <- get_pca_var(pca)
```
...
engagement_metrics_scaled$hc_cluster <- hc_cut

ggplot(engagement_metrics_scaled, aes(x = as.factor(hc_cluster), fill = as.factor(hc_cluster))) +
  geom_bar() +
...
# YOU NEED TO STANDARDIZE THESE VALUES BEFORE YOU GRAPH THEM!!!!
cluster_averages <- engagement_metrics_scaled |>
  group_by(hc_cluster) |>
  summarize(across(where(is.numeric), mean)) |>
  ungroup() |>
  pivot_longer(-hc_cluster, names_to = "metrics", values_to = "value")

ggplot(cluster_averages, aes(x = metrics, y = value, fill = metrics)) + 
  geom_col() +
...
# REMOVE hc_cluster from tibble
engagement_metrics_scaled <- engagement_metrics_scaled |> dplyr::select(-hc_cluster)
# KEEP STANDARDIZE
```
...
dims <- get_pca_ind(pca)

metrics <- engagement_metrics_scaled |>
  mutate(pca_dim1 = dims$coord[,1],
         pca_dim2 = dims$coord[,2],
         pca_dim3 = dims$coord[,3],
         pca_dim4 = dims$coord[,4])

gmm <- Mclust(dplyr::select(metrics, starts_with("pca_dim")))
...
```{r}
pca_scores <- get_pca(pca, element = "ind")
scores_df <- as.data.frame(pca_scores$coord)
scores_tibble <- as_tibble(scores_df)

engagement_metrics_scaled$gmm <- gmm$classification

classifications <- bind_cols(engagement_metrics_scaled, scores_tibble)

# classifications <- engagement_metrics_scaled |>
#   dplyr::select(starts_with("Dim"), gmm)

cluster_averages <- classifications |>
  group_by(gmm) |>
  summarize(across(where(is.numeric), mean)) |>
  ungroup() |>
  pivot_longer(-gmm, names_to = "metrics", values_to = "value")
...
```{r}
test <- Mclust(scale(engagement_metrics))
test
```
...
For averages, impute the session_avg
